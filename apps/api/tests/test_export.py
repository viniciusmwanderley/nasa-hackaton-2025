...
class TestExportDataProcessing:
    ...
    def test_create_export_rows_basic(self):
        ...
        assert (
            row1.hi_c is not None
        )  # Heat index should be calculated for hot/humid conditions
        assert row1.precip_mm_per_h == pytest.approx(
            5.0 / 24.0
...
class TestExportEndpoint:
    ...
    async def test_export_endpoint_json(self, mock_collect_samples):
        ...
        # Check JSON content
        data = (
            response.json() if hasattr(response, "json") else json.loads(response.text)
        )
        assert isinstance(data, list)
        assert len(data) >= 1
