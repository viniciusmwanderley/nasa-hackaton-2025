...
class TestCalculateProbability:
    ...
    def create_test_samples(self, 
                          ...
                          wet_fraction: float = 0.0) -> List[WeatherSample]:
        """Helper to create test samples with specified fractions of conditions."""
        samples = []
        base_date = datetime(2020, 1, 1, 12, 0, tzinfo=timezone.utc)
        ...
        for i in range(count):
            ...
            elif i < int(count * (hot_fraction + cold_fraction + windy_fraction + wet_fraction)):
            ...
            wind = 15.0 if is_windy else (12.0 if is_cold else 2.0)  # Wind threshold is 10.8 m/s
