...
from app.weather.power import PowerClient, PowerAPIError
...
class TestCollectSamples:
    ...
    @pytest.mark.asyncio
    async def test_collect_samples_basic(self, mock_power_client, sample_power_response):
        ...
        with patch('app.engine.samples.tz_for_point') as mock_tz:
            ...
            with patch('app.engine.samples.get_day_of_year') as mock_doy:
                ...
                settings = Settings(
                    coverage_min_years=1,
                    coverage_min_samples=1,
    ...
    async def test_collect_samples_insufficient_coverage(self, mock_power_client):
        ...
        mock_power_client.get_daily_data.return_value = {
            "properties": {"parameter": {"T2M": {}, "RH2M": {}, "WS10M": {}, "PRECTOTCORR": {}}}
        }
        ...
        with patch('app.engine.samples.tz_for_point') as mock_tz:
            ...
            with patch('app.engine.samples.get_day_of_year') as mock_doy:
                ...
                with pytest.raises(InsufficientCoverageError, match="Insufficient coverage"):
                    await collect_samples(
                        latitude=-3.7319,
                        longitude=-38.5267,
                        target_date_str="2024-06-15",
    ...
    async def test_collect_samples_multi_year(self, mock_power_client):
        ...
        with patch('app.engine.samples.tz_for_point') as mock_tz:
            ...
            with patch('app.engine.samples.get_day_of_year') as mock_doy:
                ...
                settings = Settings(
                    coverage_min_years=2,
                    coverage_min_samples=4,
                    coverage_enforce=False
                )
...
class TestValidateSampleCoverage:
    ...
    def test_validate_sample_coverage_inadequate(self):
        ...
        assert result["years_coverage_ratio"] == 1.0  # 1 year requested, 1 year received
