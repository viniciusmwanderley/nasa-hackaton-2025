        ...
        data_with_subset = {
            "properties": {
                "parameter": {
                    "T2M": {
                        "20230101": 25.5
                    }  # Only T2M, which is valid for custom requests
                }
            }
        }
            ...
            for param in PowerClient.REQUIRED_PARAMS:
                ...
                for date_str, value in param_data.items():
                    assert isinstance(value, (int, float)), (
                        f"Invalid {param} value type: {type(value)}"
                    )

                    # Basic sanity checks for parameter ranges
                    if param == "T2M":  # Temperature
                        assert -50 <= value <= 60, (
                            f"Temperature {value}°C seems unrealistic"
                        )
                    elif param == "RH2M":  # Humidity
                        assert 0 <= value <= 100, (
                            f"Humidity {value}% out of valid range"
                        )
                    elif param == "WS10M":  # Wind speed
                        assert 0 <= value <= 100, (
                            f"Wind speed {value} m/s seems unrealistic"
                        )
                    elif param == "PRECTOTCORR":  # Precipitation
                        assert value >= 0, (
                            f"Negative precipitation {value} mm/day invalid"
                        )
...
class TestPowerClientEdgeCases:
    ...
    async def test_single_day_request(self, client):
        ...
        with patch.object(client, "_make_request", return_value=mock_data):
            data = await client.get_daily_data(0, 0, date(2023, 6, 1), date(2023, 6, 1))
            assert "properties" in data
