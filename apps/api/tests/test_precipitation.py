...
class TestPrecipitationClient:
    ...
    def test_precipitation_client_custom_settings(self):
        ...
        settings = Settings(
            enable_imerg=False,
            enable_precipitation_fallback=True
        )
...
class TestImergPrimarySource:
    ...
    async def test_get_hourly_precipitation_imerg_success(self):
        ...
        with patch('app.clients.precipitation.create_imerg_client') as mock_create_imerg:
            # Mock IMERG client
            ...
            # Mock POWER client  
            with patch('app.clients.precipitation.create_power_client') as mock_create_power:
...
class TestPowerFallback:
    ...
    async def test_get_hourly_precipitation_imerg_failure_power_success(self):
        ...
        with patch('app.clients.precipitation.create_imerg_client') as mock_create_imerg:
            ...
            mock_imerg = AsyncMock()
            mock_imerg.get_hourly_precipitation.side_effect = Exception("IMERG API error")
            mock_create_imerg.return_value = mock_imerg
...
class TestSpecificHourRetrieval:
    ...
    async def test_get_precipitation_for_hour_found(self):
        ...
        mock_hourly_data = [
            PrecipitationData(
                hour=10, total_mm=1.0, avg_rate_mm_hr=1.0, 
                data_points=2, quality_score=0.9, source="IMERG"
            ),
            PrecipitationData(
                hour=14, total_mm=3.5, avg_rate_mm_hr=3.5,
                data_points=2, quality_score=0.9, source="IMERG"
            )
        ]
