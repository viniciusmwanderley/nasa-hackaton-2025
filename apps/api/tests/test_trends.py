...
from app.analysis.trends import (
    ...
    calculate_exceedance_rates_by_year,
    calculate_ols_slope_and_pvalue,
    calculate_trend,
)
            ...
            for i in range(n_normal):
                samples.append(
                    WeatherSample(
                        ...
                        wind_speed_ms=5.0,
                        precipitation_mm_per_day=1.0,
                        timestamp=datetime(year, 6, 10 + i),
                        latitude=-3.7,
                        longitude=-38.5,
                    )
                )
