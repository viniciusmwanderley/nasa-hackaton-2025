...
from ..weather.thresholds import WeatherSample, WeatherConditionFlags, flag_weather_conditions
...
class ProbabilityResult:
    ...
    # Metadata
    condition_type: str = "any"         # Type of condition analyzed ("any", "hot", "cold", etc.)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "metadata": {
                ...
                "relative_error": self.relative_error if math.isfinite(self.relative_error) else None
            }
        }
...
def incomplete_beta(x: float, a: float, b: float) -> float:
    ...
    bt = (math.exp(log_gamma(a + b) - log_gamma(a) - log_gamma(b) + 
...
def clopper_pearson_interval(successes: int, trials: int, confidence_level: float = 0.95) -> Tuple[float, float]:
    """
...
def calculate_probability(
    ...
    if condition_type not in valid_condition_types:
        raise ValueError(f"condition_type must be one of {valid_condition_types}, got '{condition_type}'")
    ...
    coverage_adequate = (
        coverage_years >= settings.coverage_min_years and
        total_count >= settings.coverage_min_samples
    )
...
def validate_sample_coverage(
    ...
    return {
        ...
        ) if settings.coverage_min_years > 0 and settings.coverage_min_samples > 0 else 0.0
    }