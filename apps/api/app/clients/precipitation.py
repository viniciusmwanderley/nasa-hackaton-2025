...
from app.clients.imerg import ImergHourlyData, create_imerg_client
from app.config import Settings
from app.weather.power import create_power_client
...
class PrecipitationClient:
    ...
    def __init__(self, settings: Settings | None = None):
        ...
        self.enable_imerg = (
            getattr(settings, "enable_imerg", True) if settings else True
        )
        self.enable_fallback = (
            getattr(settings, "enable_precipitation_fallback", True)
            if settings
            else True
        )
    ...
    async def get_hourly_precipitation(
        self, latitude: float, longitude: float, date: datetime, timezone: str = "UTC"
        ...
        if self.enable_imerg:
            try:
                logger.info(
                    f"Attempting IMERG data for {latitude:.2f}, {longitude:.2f}, {date.date()}"
                )
                ...
                if imerg_data:
                    logger.info(
                        f"Successfully retrieved {len(imerg_data)} hourly IMERG data points"
                    )
        ...
        if self.enable_fallback:
            try:
                ...
                logger.info(
                    f"Falling back to POWER data for {latitude:.2f}, {longitude:.2f}, {date.date()}"
                )
                ...
                if power_data and power_data.data:
                    ...
                    logger.info(
                        f"Successfully retrieved POWER fallback data: {daily_precip:.2f} mm/day"
                    )
                    return precipitation_data
                else:
    ...
    async def check_data_availability(
        ...
        """
        availability = {"imerg": False, "power": False, "any_available": False}
...
def create_precipitation_client(
