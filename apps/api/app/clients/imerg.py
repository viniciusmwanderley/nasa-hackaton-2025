...
class ImergDataPoint(BaseModel):
    ...
    quality_flag: Optional[int] = Field(default=None, description="Quality indicator")
...
class ImergClient:
    ...
    async def get_half_hourly_data(
        ...
        try:
            ...
            for half_hour in range(48):  # 48 half-hour periods in a day
                ...
                import random
                random.seed(int(timestamp.timestamp()) + int(latitude * 1000) + int(longitude * 1000))
                variability = random.uniform(0.5, 2.0)
                
                precipitation = base_precip * variability
                quality = random.uniform(0.7, 1.0)
                
                data_points.append(ImergDataPoint(
    ...
    async def aggregate_to_hourly(
        self,
        half_hourly_data: ImergResponse,
        timezone: str = "UTC"
        ...
        try:
            ...
            for hour in sorted(hourly_data.keys()):
                ...
                results.append(ImergHourlyData(
                    hour=hour,
                    total_mm=total_mm,
            ...
            logger.info(f"Aggregated {len(half_hourly_data.data)} half-hourly points to {len(results)} hourly values")
            return results
...
def create_imerg_client(settings: Optional[Settings] = None) -> ImergClient:
