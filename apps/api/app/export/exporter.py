...
def create_export_rows(
    sample_collection: SampleCollection, settings: Settings
    ...
    for sample in sample_collection.samples:
        ...
        if (
            hasattr(sample, "precipitation_mm_hourly")
            and sample.precipitation_mm_hourly is not None
        ):
...
def generate_export_filename(
    latitude: float, longitude: float, target_date: str, target_hour: int, format: str
) -> str:
    """
...
def validate_export_data(export_rows: list[ExportSampleRow]) -> dict[str, Any]:
    ...
    coordinate_consistency = all(
        row.lat == first_lat and row.lon == first_lon for row in export_rows
    )
