...
from app.engine.samples import InsufficientCoverageError, collect_samples
...
async def assess_risk(request: Request, risk_request: RiskRequest):
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"Insufficient coverage: {e}", extra={"request_id": request_id}
            )
            raise HTTPException(
                status_code=422, detail=f"Insufficient historical data coverage: {e}"
            )
        except ValueError as e:
            logger.error(
                f"Invalid request parameters: {e}", extra={"request_id": request_id}
            )
            raise HTTPException(status_code=400, detail=str(e))
        except Exception as e:
            logger.error(
                f"Sample collection failed: {e}", extra={"request_id": request_id}
            )
            raise HTTPException(
                status_code=500, detail="Failed to collect weather samples"
            )
        ...
        for condition in conditions:
            try:
                ...
                logger.error(
                    f"Probability calculation failed for {condition}: {e}",
                    extra={"request_id": request_id},
                )
                raise HTTPException(
                    status_code=500,
        ...
        if risk_request.detail == "full":
            logger.info(
                "Calculating distributions and trends for full response",
                extra={"request_id": request_id},
            )
            ...
            try:
                ...
                response = RiskResponseFull(
                    **base_data, distributions=distributions, trends=trends
                )

            except Exception as e:
                logger.error(
                    f"Failed to calculate distributions/trends: {e}",
                    extra={"request_id": request_id},
                )
        ...
        logger.error(
            f"Unexpected error in risk assessment: {e}",
            extra={"request_id": request_id},
        )
        raise HTTPException(
            status_code=500, detail="Internal server error during risk assessment"
        )
...
async def export_data(request: Request, export_request: ExportRequest):
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"Insufficient coverage for export: {e}",
                extra={"request_id": request_id},
            )
            raise HTTPException(
                status_code=422, detail=f"Insufficient historical data coverage: {e}"
            ...
            logger.error(
                f"Invalid export parameters: {e}", extra={"request_id": request_id}
            )
            raise HTTPException(status_code=400, detail=str(e))
        except Exception as e:
            logger.error(
                f"Sample collection failed for export: {e}",
                extra={"request_id": request_id},
            )
            raise HTTPException(
                status_code=500, detail="Failed to collect weather samples for export"
            )
        ...
        try:
            ...
            if not validation_result["valid"]:
                ...
                raise HTTPException(
                    status_code=500, detail="Export data validation failed"
                )
            ...
            logger.error(
                f"Export processing failed: {e}", extra={"request_id": request_id}
            )
            raise HTTPException(status_code=500, detail="Failed to process export data")
        ...
        logger.error(
            f"Unexpected error in data export: {e}", extra={"request_id": request_id}
        )
        raise HTTPException(
            status_code=500, detail="Internal server error during data export"
        )
