...
from ..weather.power import PowerClient, PowerAPIError
from ..time.timezone import tz_for_point, to_local, get_day_of_year, parse_date
...
class WeatherSample:
    ...
    precipitation_mm_hourly: Optional[float] = None  # Hourly precipitation at target hour
    precipitation_source: str = "POWER"  # "IMERG" or "POWER_fallback"
    precipitation_quality: Optional[float] = None  # Quality score 0-1
...
async def collect_samples(
    ...
    if baseline_start_year > baseline_end_year:
        raise ValueError(f"Invalid baseline years: {baseline_start_year} > {baseline_end_year}")
    ...
    for year in range(baseline_start_year, baseline_end_year + 1):
        try:
            ...
            raise SampleCollectorError(f"Failed to collect samples for year {year}: {e}") from e
    
    # Check coverage requirements
    coverage_adequate = (
        years_with_data >= settings.coverage_min_years and
        len(all_samples) >= settings.coverage_min_samples
    )
...
async def _collect_samples_for_year(
    ...
    precipitation_client: Optional[Any],
    latitude: float,
    longitude: float,
    year: int,
    target_doy: int,
    ...
    logger.debug(f"Fetching POWER data for year {year}, DOY {start_doy}-{end_doy} ({start_date} to {end_date})")
    ...
    for date_key in t2m_data.keys():
        try:
            ...
            sample_datetime_utc = datetime.combine(sample_date, datetime.min.time().replace(hour=12))
            ...
            target_datetime_utc = datetime.combine(sample_date, datetime.min.time().replace(hour=target_hour))
            ...
            if any(value is None for value in [temperature_c, relative_humidity, wind_speed_ms]):
            ...
            hourly_precip = precipitation_mm_per_day / 24.0 if precipitation_mm_per_day > 0 else 0.0
