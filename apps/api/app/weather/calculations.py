...
def heat_index(temperature_c: float, relative_humidity: float) -> Optional[float]:
    """
    ...
    if not (0 <= relative_humidity <= 100):
        raise ValueError(f"Relative humidity {relative_humidity}% must be between 0-100%")
...
def wind_chill(temperature_c: float, wind_speed_ms: float) -> Optional[float]:
    """
...
def feels_like_temperature(temperature_c: float, 
                          relative_humidity: Optional[float] = None,
                          wind_speed_ms: Optional[float] = None) -> float:
    """
...
def validate_temperature_range(temperature_c: float, min_temp: float = -100, max_temp: float = 100) -> None:
    """
