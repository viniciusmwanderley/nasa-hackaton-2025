...
class PowerClient:
    ...
    def __init__(self, settings: Settings | None = None):
        ...
        self.client = httpx.AsyncClient(
            timeout=timeout,
            headers={"User-Agent": "outdoor-risk-api/0.1.0 (NASA Hackathon 2025)"},
        )
    ...
    async def _make_request(self, url: str, params: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"POWER API HTTP error {e.response.status_code}: {e.response.text}"
            )
...
async def create_power_client(settings: Settings | None = None) -> PowerClient:
    """
