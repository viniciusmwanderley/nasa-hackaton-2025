...
class RiskRequest(BaseModel):
    ...
    detail: str = Field(
        default="lean", 
        description="Response detail level: 'lean' or 'full'"
    )
...
class ConditionProbability(BaseModel):
    ...
    positive_samples: int = Field(description="Number of samples with flagged conditions")
...
class SampleStatistics(BaseModel):
    ...
    coverage_adequate: bool = Field(description="Whether coverage meets minimum requirements")
...
class ConditionThresholds(BaseModel):
    ...
    very_hot_c: float = Field(description="Heat index threshold for very hot conditions (°C)")
    very_cold_c: float = Field(description="Wind chill threshold for very cold conditions (°C)")
    very_windy_ms: float = Field(description="Wind speed threshold for very windy conditions (m/s)")
    very_wet_mm_per_h: float = Field(description="Precipitation rate threshold for very wet conditions (mm/h)")
...
class RiskResponseLean(BaseModel):
    ...
    very_hot: ConditionProbability = Field(description="Probability of very hot conditions")
    very_cold: ConditionProbability = Field(description="Probability of very cold conditions")
    very_windy: ConditionProbability = Field(description="Probability of very windy conditions")
    very_wet: ConditionProbability = Field(description="Probability of very wet conditions")
    any_adverse: ConditionProbability = Field(description="Probability of any adverse conditions")
    
    # Sample metadata
    sample_statistics: SampleStatistics = Field(description="Statistics about the sample collection")
...
class HistogramBin(BaseModel):
    ...
    upper_bound: float = Field(description="Upper bound of bin (exclusive, except for last bin)")
...
class Distribution(BaseModel):
    ...
    threshold_value: Optional[float] = Field(
        None, 
        description="Threshold value for this parameter (if applicable)"
    )
...
class Trend(BaseModel):
    ...
    significant: bool = Field(description="Whether trend is statistically significant (p < 0.05)")
